...
describe('building service', function () {
  ...
  it('createRoom roomService + Room integration test using roomRepository stub', async function () {	
		...
		// Assert
		sinon.assert.calledOnce(res.json);
		sinon.assert.calledWith(res.json, sinon.match({"id":"123","floorId": req.body.floorId,"name": req.body.name,"category": req.body.category,"description": req.body.description,"posX": req.body.posX,"posY": req.body.posY,"width": req.body.width,"height": req.body.height}));
	});
